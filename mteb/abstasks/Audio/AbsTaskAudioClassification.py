...
class AbsTaskAudioClassification(AbsTask):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.samples_per_label: int = (  # type: ignore
            ...
            else self.metadata_dict.get("samples_per_label", 50) # replaced 16
        )
    ...
    def _undersample_data(
        ...
        if idxs is None:
            idxs = np.arange(len(dataset_split)) #generate indices for training dataset
        np.random.shuffle(idxs)
        if not isinstance(idxs, list):
            idxs = idxs.tolist() # make into list
        ...
        for i in idxs:
            label = labels[i]
            if label_counter[label] < samples_per_label: #if num of samples not hit
